...
sealed interface DataResult<T: Any> {
     ...
     */
    data class Success<T: Any>(val data: T): io.github.mattshoe.shoebox.kernl.data.DataResult<T>

    /**
     * Encapsulates an error encountered by a data retrieval operation.
     */
    data class Error<T: Any>(val error: Throwable):
        io.github.mattshoe.shoebox.kernl.data.DataResult<T>

    /**
     * Representation of an invalidated cache. Meaning any previous data
     * is invalid.
     */
    data class Invalidated<T: Any>(private val data: Unit = Unit): io.github.mattshoe.shoebox.kernl.data.DataResult<T>
}